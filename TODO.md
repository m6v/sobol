...
2. Сделать класс со списком пользователей UserList и их настройками. Методы remove_user(user), remove_all_users(), add_user(user), get_user(user_id)
3. Настройках хранить не учетные записи, а идентификаторы зарегистрированных iButton. Данные об учетках хранить в настройках ibutton2dbus и передавать через dbus
4. Разобраться с ошибками при множественном наследовании QVNCWidget(QWidget, RFBClient). В PyQt5 работает нормально, а в PySide2 нет! В итоге пришлось вставлять костыли и сейчас хотя и работает, но выдает ошибки
