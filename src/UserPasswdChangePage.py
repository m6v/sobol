...
CONFIRM_PASSWD_CHANGE_PAGE = 0
OLD_PASSWD_ENTRY_PAGE = 1
NEW_PASSWD_ENTRY_PAGE = 2
WAIT_ID_PAGE = 3
COMPLETION_PAGE = 4
...
class UserPasswdChangePage(QtWidgets.QWidget):
    ...
    def on_ibutton_presented(self, message):
        ...
        # TODO Здесь нужно дополнить проверку еще одного уловия: соответствия имени пользователя,
        # для которого изменяем пароль, имени в предъявленной ibutton!
        if not self.is_forced_passwd_change and self.old_passwd_line_edit.text() != message["passwd"]:
    ...
    def showEvent(self, event: QShowEvent):
        # В зависимости от того принудительная смена пароля
        # или нет открыть нужную страницу мастера
        if self.is_forced_passwd_change:
            self.stacked_widget.setCurrentIndex(CONFIRM_PASSWD_CHANGE_PAGE)
        else:
            self.stacked_widget.setCurrentIndex(OLD_PASSWD_ENTRY_PAGE)
